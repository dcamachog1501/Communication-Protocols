...
module UART_RX(
    ...
    start_detector strt_detect(.DATA_IN(DATA_IN),.CHECK_START(check_start),.START_DETECTED(start_detected));
    ...
    stop_detector stp_detect(.DATA_IN(DATA_IN),.CHECK_STOP(check_stop),.STOP_BIT_ERROR(STOP_BIT_ERROR));
